...
class NetworkMapWidget(QWidget):
    ...
    def cleanup(self):
        ...
        # Stop animated items (blinking arrows and beams)
        for item in self.animated_items[:]:  # Copy list to avoid modification during iteration
            try:
    ...
    def force_refresh(self):
        """Force refresh network map - panggil setelah session di-kill"""
        try:
            # Cleanup animated items terlebih dahulu
            self.cleanup()
            # Refresh map
            self.refresh_map()
        except Exception as e:
            print(f"[ERROR] Network map force refresh: {e}")