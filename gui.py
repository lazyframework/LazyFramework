...
from PyQt6.QtWidgets import QComboBox  # ← Harus ada!
...
# === AKTIFKAN FAULTHANDLER UNTUK DEBUG CRASH ===
faulthandler.enable()
# Dump ke file juga (lebih aman)
faulthandler.enable(file=open("crash.log", "a"))

print("[DEBUG] faulthandler enabled - crash.log akan dibuat jika crash")
...
class LazyFrameworkGUI(QMainWindow):
    ...
    session_killed_signal = pyqtSignal(str)  # <-- TAMBAHKAN IN
    def __init__(self):
        super().__init__()
        ...
        self.session_killed_signal.connect(self.on_session_killed)  # <-- TAMBAHKAN BARIS INI
    ...
    def on_session_killed(self, session_id):
        """Handle session killed signal - refresh network map"""
    ...
    def on_module_selected(self, item):
        """Single click: Tampilkan info di Info tab + update toolbar"""
        if not item:
            return
    ...
    def _populate_module_tab_options(self, tab: "ModuleTab", mod_instance):
        ...
        # Shared stylesheet for all input widgets
        INPUT_STYLE = """
            QLineEdit, QComboBox {
                background: #2d2d2d;
                color: #cccccc;
        ...
        try:
            ...
            for key, meta in options.items():
                ...
                if isinstance(meta, dict):
                    ...
                    choices = meta.get('choices')  # list or None
                ...
                if choices:
                    ...
                    # Set to current/default value if valid
    ...
    def _update_toolbar_module(self, module_path: str, active: bool = True):
        """Update label nama module di toolbar + state tombol Run/Back"""
        ...
        if module_path:
            ...
            label_style = (
                ...
                if active else
                "color: #9cdcfe; font-size: 10pt; font-weight: 500;"
            )
            ...
            if hasattr(self, '_toolbar_dot'):
                self._toolbar_dot.setStyleSheet("color: #444; font-size: 10px; margin-right: 2px;")
            if hasattr(self, 'run_btn'):
    ...
    def create_menu_bar(self):
        ...
        settings_menu = menubar.addMenu('Settings')
            # === TAMBAHKAN INI: Theme Submenu ===
        theme_menu = settings_menu.addMenu("🎨 Theme")
        
        # Scan themes dari theme_manager
        available_themes = self.theme_manager.get_available_themes()
        current_theme_display = None
        
        # Cari nama display dari current theme
        ...
        for display_name in available_themes:
            ...
            theme_action.triggered.connect(lambda checked, name=display_name: self.switch_theme(name))
    ...
    def create_main_content(self):
        """Create main content area - redesign VSCode style, satu area gabungan"""
        main_widget = QWidget()
        ...
        # ── TOOLBAR TUNGGAL ──────────────────────────────────────────────────
        # Satu bar saja: module name | [spacer] | START | BACK | Clear
        toolbar = QWidget()
        ...
        # Indikator modul aktif (dot + nama)
        self._toolbar_dot = QLabel("●")
        self._toolbar_dot.setStyleSheet("color: #555; font-size: 10px; margin-right: 2px;")
        ...
        #self.console_output.setStyleSheet("background: #1e1e1e; border: none; padding: 4px;")
        self.main_tabs.addTab(self.console_output, "Console")

        # ── Tab: Options ──
        self.options_widget = QWidget()
        #self.options_widget.setStyleSheet("background: #1e1e1e;")
        self.options_layout = QFormLayout(self.options_widget)
        self.options_layout.setContentsMargins(16, 12, 16, 12)
        self.options_layout.setSpacing(10)
        self.options_layout.setLabelAlignment(Qt.AlignmentFlag.AlignRight)
        self.options_scroll = QScrollArea()
        self.options_scroll.setWidgetResizable(True)
        #self.options_scroll.setStyleSheet("QScrollArea { border: none; background: #1e1e1e; }")
        ...
        #self.module_detail_info.setStyleSheet("background: #1e1e1e; border: none; padding: 4px;")
        self.main_tabs.addTab(self.module_detail_info, "Info")

        # ── Tab: Sessions ──
        self.session_tab = QWidget()
        #self.session_tab.setStyleSheet("background: #1e1e1e;")
        ...
        protected_tabs = ["Console", "Options", "Info", "Sessions", "Network Map", "AI Assistant"]
        for i in range(self.main_tabs.count()):
            tab_text = self.main_tabs.tabText(i)
            clean_text = tab_text.lstrip("⚡● ").strip()
            if clean_text in protected_tabs:
                self.main_tabs.tabBar().setTabButton(i, QTabBar.ButtonPosition.RightSide, None)
        
        

        # Sembunyikan tombol close di tab bawaan (indeks tetap)
        self._protected_tab_count = self.main_tabs.count()

        layout.addWidget(self.main_tabs)

        # ── module_tabs alias (agar kode lain tidak crash) ──────────────────
        # module_tabs sekarang menunjuk ke main_tabs; tab module baru
        # tetap dibuka di sini via open_module_in_tab / open_module_in_main_tab
    ...
    def switch_theme(self, display_name):
        ...
        if display_name in self.theme_manager.theme_map:
            ...
            #self.append_output(f"[green]✓ Theme switched to: {display_name}[/]")
            
            # Update checkmarks di menu
            self.update_theme_menu_checks(display_name)

    def update_theme_menu_checks(self, active_display_name):
        """Update checkmark di theme menu"""
        # Cari Settings menu
    ...
    def reload_themes(self):
        """Reload themes from folder"""
        self.theme_manager.theme_map = self.theme_manager._scan_themes()
        #self.append_output(f"[green]✓ Themes reloaded: {len(self.theme_manager.theme_map)} found[/]")
        

    def _close_main_tab(self, index: int):
            """Tutup tab modul dinamis; tab bawaan tidak bisa ditutup."""
            protected = {"Console", "Options", "Info", "Sessions", "Network Map", "AI Assistant"}
            try:
                title = self.main_tabs.tabText(index)
                # Hapus prefix icon jika ada (misal "⚡ sqlmap" → "sqlmap")
    ...
    def create_left_sidebar(self):
        """Create left sidebar with modules and categories - WITH SCROLLABLE CATEGORIES"""
        sidebar = QWidget()
        ...
        # ============================================================
        # Container untuk category buttons dengan scroll horizontal
        ...
        # Daftar kategori lengkap (15 tombol)
        categories = [
            ("All",      "all",      "#8be9fd", "📋"),
            ("Recon",    "recon",    "#8be9fd", "📡"),
            ("Scan",     "scan",     "#50fa7b", "🔍"),
        ...
        # Scroll Area untuk categories (horizontal scroll)
        ...
        categories_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        categories_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        ...
        # ============================================================
        # SPLITTER untuk Module Tree + Info/Browser
        # ============================================================
        splitter = QSplitter(Qt.Orientation.Vertical)

        # === QTreeWidget sebagai module explorer ===
    ...
    def load_module_to_main_tab(self, module_path: str):
        ...
        try:
            ...
            if self.framework.loaded_module:
                ...
                #self.append_output(f"[green] module: {module_path.split('/')[-1]} in main tab[/]")
    ...
    def append_output(self, text):
        """Append output with full Rich + ANSI + nmap coloring"""
        ...
        # IMPORTANT: Save raw text BEFORE cleaning for session detection
        raw_text = text

        # Render with full color processing
        ...
        # === 3. DETEKSI SESSION BARU (gunakan raw_text) ===
        session_patterns = [
            r"Session (.+?) opened \((.+?) -> (.+?)\)",
            r"\[\+\]\s+Session (.+?) opened",
        ...
        if detected and self.main_tabs.currentIndex() != 3:
            ...
            self.append_output("[red]Switching to control panel...[/]")

    def strip_ansi_sequences(self, text):
        """Bersihkan semua ANSI escape sequences"""
        import re
    ...
    def rich_to_html_with_matrix(self, text):
        """Convert rich/ANSI/nmap text to colored HTML with Matrix theme"""
        ...
        ANSI = {
            '0':'reset','1':'bold','30':'black','31':'red','32':'green',
        ...
        # 2. Convert bare or ESC-prefixed ANSI color codes → Rich-style tags
        def ansi_to_rich(m):
        ...
        # strip leftover ESC-only sequences
        text = re.sub(r'\x1b\[[0-9;]*[a-zA-Z]', '', text)
        text = text.replace('\x1b', '')

        # 3. Escape HTML special chars (but not inside spans we'll build)
        text = text.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')

        # 4. Nmap keyword coloring (applied line-by-line before Rich processing)
        ...
        # 6. Box-drawing chars
        for ch, col in [('│','#00ff00'),('─','#00ff00'),('┌','#00ff00'),('┐','#00ff00'),
    ...
    def format_unicode_table(self, text):
        """Format unicode table dengan Matrix theme - with word wrap"""
        # Escape HTML
        safe = (
            text.replace("&", "&amp;")
                .replace("<", "&lt;")
        ...
        # GANTI: pake div dengan overflow-x auto, tapi tetap wrap
        html = f"""
        <div style="
            max-width: 100%;
            overflow-x: auto;
    ...
    def create_new_session(self, match, raw_text):
        """Create new session from detected pattern - DENGAN DETEKSI OS & HOSTNAME"""
        try:
            ...
            # OS Detection
            if any(keyword in text_lower for keyword in ['linux', 'unix', 'ubuntu', 'debian', 'centos', 'kali', 'parrot']):
                detected_os = 'linux'
            elif any(keyword in text_lower for keyword in ['windows', 'microsoft', 'cmd.exe', 'powershell', 'win32', 'win64']):
                detected_os = 'windows' 
            elif any(keyword in text_lower for keyword in ['macos', 'darwin', 'apple', 'mac os']):
                detected_os = 'macos'

            # Buat session data dengan info OS dan hostname
            session_data = {
                'id': sess_id,
                'type': 'reverse_tcp',
                'lhost': dst_ip,
    ...
    def sync_sessions_from_reverse_tcp(self):
        """Sync GUI sessions from reverse_tcp module - FIXED dengan hostname"""
        try:
            ...
            with SESSIONS_LOCK:
                for sess_id, rev_sess in SESSIONS.items():
                    ...
                    # Get hostname - direct from session object
                    ...
                    if sess_id not in self.sessions:
                        ...
                        self.sessions[sess_id] = {
                            ...
                            'os': rev_os,
                            'hostname': rev_hostname if rev_hostname != 'unknown' else f"target_{sess_id[-4:]}",
                        ...
                        self.append_output(f"[green]✓ Added session {sess_id} (hostname: {rev_hostname})[/]")
                    else:
    ...
    def kill_selected_session(self):
        """Kill selected session - FIXED VERSION dengan network map refresh"""
        item = self.session_list.currentItem()
        if not item:
        ...
        # === METHOD 1: Coba kill via reverse_tcp module jika loaded ===
        try:
            # Cari module reverse_tcp yang sedang berjalan
            target_module = None
            if (self.framework.loaded_module and 
            ...
            if target_module and hasattr(target_module, 'kill_session'):
                ...
                if success:
                    self.append_output(f"[green]✓ Session {sess_id} killed via module[/]")
        except Exception as e:
            self.append_output(f"[yellow]Module kill error: {e}[/]")
        
        # === METHOD 2: Hapus dari GUI sessions ===
    ...
    _CAT_META = {
        ...
        "scan":       {"color": "#50fa7b", "icon": "🔍", "label": "Scan"},        # BARU
        "exploit":    {"color": "#ff5555", "icon": "⚡", "label": "Exploit"},
        "post":       {"color": "#f1fa8c", "icon": "🎯", "label": "Post"},         # BARU
        "privesc":    {"color": "#ffb86c", "icon": "⬆️", "label": "Privesc"},      # BARU
        "persistence":{"color": "#bd93f9", "icon": "🔒", "label": "Persistence"},  # BARU
        "lateral":    {"color": "#ff79c6", "icon": "🔄", "label": "Lateral"},      # BARU
        "web":        {"color": "#00ffff", "icon": "🌐", "label": "Web"},          # BARU
        "cloud":      {"color": "#ffffff", "icon": "☁️", "label": "Cloud"},        # BARU
        "mobile":     {"color": "#aaffaa", "icon": "📱", "label": "Mobile"},       # BARU
        "aux":        {"color": "#888888", "icon": "🛠️", "label": "Aux"},          # BARU
        "payloads":   {"color": "#bd93f9", "icon": "💣", "label": "Payloads"},
        "evasion":    {"color": "#ffaa00", "icon": "👻", "label": "Evasion"},      # BARU
        "report":     {"color": "#4a90e2", "icon": "📊", "label": "Report"},       # BARU
    ...
    def _detect_category(self, module_path: str) -> str:
        ...
        if "/scan/"        in p: return "scan"          # BARU
        if "/exploit/"     in p: return "exploit"
        if "/post/"        in p: return "post"          # BARU
        if "/privesc/"     in p: return "privesc"       # BARU
        if "/persistence/" in p: return "persistence"   # BARU
        if "/lateral/"     in p: return "lateral"       # BARU
        if "/web/"         in p: return "web"           # BARU
        if "/cloud/"       in p: return "cloud"         # BARU
        if "/mobile/"      in p: return "mobile"        # BARU
        if "/aux/"         in p: return "aux"           # BARU
        if "/payload/"     in p: return "payloads"
        if "/evasion/"     in p: return "evasion"       # BARU
        if "/report/"      in p: return "report"        # BARU
        return "other"
    ...
    def load_all_modules(self):
        ...
        CAT_ORDER = ["recon", "scan", "exploit", "post", "privesc", 
    ...
    def load_selected_module(self, item):
        """Double click: Buka module sebagai tab baru + update toolbar"""
        if not item:
            return
        module_path = item.data(0, Qt.ItemDataRole.UserRole)
        if not module_path:
            ...
            return
        # Update toolbar label (aktif)
        self._update_toolbar_module(module_path, active=True)
        # Update Info tab
        self.load_module_info_to_main_tab(module_path)
        # Buka sebagai tab module di area utama
    ...
    def load_module_to_main_tab(self, module_path: str):
        ...
        try:
            ...
            if self.framework.loaded_module:
                # Tampilkan module_path langsung (sudah dalam format recon/nmap)
                # Hapus prefix "modules/" jika ada
                display_path = module_path
                if display_path.startswith('modules/'):
                    display_path = display_path[8:]  # Hapus "modules/"
                ...
                #self.append_output(f"[green]✓ Loaded module: {display_path}[/]")
    ...
    def on_module_loaded(self):
        ...
        if self.framework.loaded_module:
            # Ambil path lengkap module (contoh: recon/nmap)
            module_full_path = self.framework.loaded_module.name
            
            # Hapus ekstensi .py jika ada
            if module_full_path.endswith('.py'):
                module_full_path = module_full_path[:-3]
            
            # Hapus prefix "modules/" jika ada
            if module_full_path.startswith('modules/'):
                module_full_path = module_full_path[8:]  # Hapus "modules/"
            
            self.current_module = module_full_path
            self.current_module_label.setText(module_full_path)  # Menampilkan recon/nmap
            self.current_module_label.setStyleSheet(
